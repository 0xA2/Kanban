...
void addTask(tasklist* todo, tasklist* doing, tasklist* done, tasklist* all){
	...
	char* desc = (char*)malloc(1024); readString(desc);
...
void workOnTask(tasklist* todo, tasklist* doing){
	...
	int day; readInt(&day);
		switch(month){
			case 1:
				if(day < 1 || day > 31){puts("Invalid date"); exit(1);}
				break;
			case 2:
				if(day < 1 || day > 29){puts("Invalid date"); exit(1);}
				break;
			case 3:
				if(day < 1 || day > 31){puts("Invalid date"); exit(1);}
				break;
			case 4:
				if(day < 1 || day > 30){puts("Invalid date"); exit(1);}
				break;
			case 5:
				if(day < 1 || day > 31){puts("Invalid date"); exit(1);}
				break;
	...
	if( (toMove = listRemoveTaskByID(id,todo)) != NULL){
		...
		char* worker = (char*)malloc(1024); readString(worker);
...
void tasksFromWorker(tasklist* doing, tasklist* done){
	...
	char* name = (char*)malloc(1024); readString(name);


	if(!listIsEmpty(doing)){
		// Print tasks worker is currently doing
		node* n1 = doing -> first; int flag1 = 1;
		if(n1 != NULL){
			puts("\nWorking on:");
			while(n1 -> next != NULL){
				if(!strcmp(name,n1 -> task -> person)){
					flag1 = 0;
					printf("\n\tID: %d | Priority: %d\n\t%s\n\tAssigned to %s", n1 -> task -> id, n1 -> task -> priority, n1 -> task -> description, n1 -> task -> person);
					n1 = n1 -> next;
				}
			}
			if(!strcmp(name,n1 -> task -> person)){
	...
	if(!listIsEmpty(done)){
		...
		node* n2 = done -> first; int flag2 = 1;
		if(n2 != NULL){
			puts("\nTasks completed:");
			while(n2 -> next != NULL){
				if(!strcmp(name,n2 -> task -> person)){
					flag2 = 0;
					printf("\n\tID: %d | Priority: %d\n\t> %s\n\tAssigned to %s", n2 -> task -> id, n2 -> task -> priority, n2 -> task -> description, n2 -> task -> person);
					n2 = n2 -> next;
				}
			}
			if(!strcmp(name,n2 -> task -> person)){
...
int main(){
	...
	char title[] = "Kanban";
	char* options[] = {"Add Task", "Work on Task", "Reassign task", "Close task", "Reopen task", "View tasks by worker", "View tasks by date", "Exit"};
